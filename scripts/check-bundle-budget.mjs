...
async function loadInputs() {
	...
	// `.size-limit.json`'s `ignore` array is the runtime source of
	// truth for which deps the host provides — we externalize those
	// from the bundle measurement so we count only what this plugin
	// actually ships. Both files MUST stay in sync; the byte budget
	// is duplicated by design (each tool reads its own file) but the
	// ignore set lives in one place.
	const [pkgRaw, configRaw, sizeLimitRaw] = await Promise.all([
	...
	const ignoreList = Array.isArray(sizeLimit) && sizeLimit[0]?.ignore
		? sizeLimit[0].ignore
		: [];

	return { pkg, config, ignoreList };
}
...
async function bundle(packageName, peerDependencies, ignoreList) {
...
async function main() {
	const { pkg, config, ignoreList } = await loadInputs();
	await ensureDistExists();
	await prepareEntry(pkg.name);

	const metafile = await bundle(pkg.name, pkg.peerDependencies ?? {}, ignoreList);
